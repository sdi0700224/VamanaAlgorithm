...
using PII = pair<uint32_t, uint32_t>; // <start_id, num_points>
...
private:
    size_t k_;
    priority_queue<pair<DisT, IdT>> pq;
};
...
inline void Bruteforce(const vector<vector<float>> &nodes, const vector<vector<float>> &queries, vector<vector<uint32_t>> &gt)
{
    ...
    for (size_t i = 0; i < nq; i++)
    {
        futures.emplace_back(pool->push([&, i]()
                                ...
                                // Populate `gt[i]` with elements from the heap
                                int j = K - 1;
...
inline float GetKNNRecall(const vector<vector<uint32_t>> &knns, const vector<vector<uint32_t>> &gt)
{
    ...
    ofstream out("recall1.bin", ios::binary);
...
inline void ReadBase(const string &source_path, uint32_t &N, vector<float> &vecs, vector<float> &labels, vector<float> &timestamps)
{
    ...
    ifstream ifs;
    ifs.open(source_path, ios::binary);
    assert(ifs.is_open());
    ifs.read((char *)&N, sizeof(uint32_t));
...
//    ifs.read((char *)&N, sizeof(uint32_t));
//
//    auto tmp_vecs = make_unique<float[]>(N * vec_dim);
//    auto tmp_labels = make_unique<float[]>(N);
//    auto tmp_timestamps = make_unique<float[]>(N);
//    vecs.resize(N * vec_dim);
//    labels.resize(N);
//    timestamps.resize(N);
//
...
//    iota(sorted_base_ids.begin(), sorted_base_ids.end(), 0);
...
//        }
//    };
//    sort(sorted_base_ids.begin(), sorted_base_ids.end(), cmp);
                                    ...
                                    unordered_map<int32_t, PII> &timestamp_map, int &max_count, int &min_count)
{
    ...
    iota(sorted_base_ids.begin(), sorted_base_ids.end(), 0);
    iota(sorted_base_ids_by_time.begin(), sorted_base_ids_by_time.end(), 0);
    iota(sorted_base_ids_by_full_time.begin(), sorted_base_ids_by_full_time.end(), 0);
    auto cmp = [&](const uint32_t &a, const uint32_t &b)
    {
        int32_t ca = tmp_labels[a];
        int32_t cb = tmp_labels[b];
    ...
    auto cmp_by_time = [&](const uint32_t &a, const uint32_t &b)
    {
        ...
        int32_t cb = tmp_labels[b];
        if (ca == maxc_id)
        { // largest category first to avoid redundant build of largest category
            if (ca == cb)
            {
                return tmp_timestamps[a] < tmp_timestamps[b];
            }
    ...
    sort(sorted_base_ids.begin(), sorted_base_ids.end(), cmp);
    sort(sorted_base_ids_by_time.begin(), sorted_base_ids_by_time.end(), cmp_by_time);
    sort(sorted_base_ids_by_full_time.begin(), sorted_base_ids_by_full_time.end(), cmp_by_full_time);
    // for the largest category, we force its order in both id map to be the same, (unstablity of sort)
    // so we don't have to change other strategy for the largest category
    int start_id = find_if(sorted_base_ids.begin(), sorted_base_ids.end(), [&](uint32_t id)
    ...
    for (size_t i = 0; i < N; ++i)
    {
        ...
        int32_t v = labels[i];
        if (i < category_end || (int)category_count[v] < min_cat_count)
        {
            continue;
        }
        if (category_map.find(v) == category_map.end())
    ...
    {
        auto s = lower_bound(timestamps_by_full_time.begin(), timestamps_by_full_time.end(), 0.1 * i);
        auto e = upper_bound(timestamps_by_full_time.begin(), timestamps_by_full_time.end(), 0.1 * (i + interval));
        int num = e - s;
        if (num == 0)
            continue;
                            ...
                            unordered_map<int32_t, vector<int>> &category_query, const unordered_map<int32_t, PII> &category_map)
{
    ...
    ifstream ifs;
    ifs.open(query_path, ios::binary);
    assert(ifs.is_open());
    ifs.read((char *)&N, sizeof(uint32_t));

    auto tmp_vecs = make_unique<float[]>(N * vec_dim);
    auto tmp_metas = make_unique<float[]>(N * 4);
    vecs.resize(N * vec_dim);
    metas.resize(4 * N);
    cout << "# of points: " << N << endl;
    for (size_t i = 0; i < N; i++)
    ...
    sort(sorted_ids.begin(), sorted_ids.end(), cmp);
    for (size_t i = 0; i < N; ++i)
    {
...
inline void ReadStats(uint32_t nq)
{
    cout << "=== Reading Query Stats ===\n";
    vector<QueryStats> q;
    q.resize(nq);
    ifstream inFile("query_stats.bin", ios::binary); // 以二进制模式打开文件
...
#pragma omp parallel for num_threads(32)
    for (int i = 0; i < N; i++)
    {
        for (int j = i + 1; j < N; j++)
        {
            ...
            {
                throw runtime_error("ip overflow");
            }
        }
    }
}